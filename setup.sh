...
if [[ "${NO_XORG}" == "true" ]]; then
	echo "-	Skipping xorg (NO_XORG is ${NO_XORG})"
else
	# Create xorg profile	
	echo "-	Creating xorg profile"
	...
	if [[ "${NO_KDE}" == "true" ]]; then
		echo "-	Skipping KDE (NO_KDE is ${NO_KDE})"
	else
		# Create kde plasma profile	
		echo "-	Creating KDE5 profile"
		...
		echo "source ~/.bashrc" >> "${HOME}/.config/plasma-workspace/env/xprofile.sh"
	fi
fi
